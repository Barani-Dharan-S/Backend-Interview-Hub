...
export default function QuestionCard({ q, index, open, onToggle, isReviewed = false, onReview = () => {} }) {
  const followups = q.followups || [];
  const mistakes = q.mistakes || [];

  return (
    <article className={open ? "question open" : "question"}>
      <button type="button" className="question-head" onClick={onToggle} aria-expanded={open} aria-controls={`question-body-${q.id}`} title={open ? "Collapse answer" : "Expand answer"}>
      ...
      {open && (
        <div id={`question-body-${q.id}`} className="answer">
          ...
          {followups.length > 0 && (
            <Section label={q.topic === "coding" ? "09 · EXPECTED FOLLOW-UPS" : (q.scenario ? "06 · EXPECTED FOLLOW-UPS" : "05 · EXPECTED FOLLOW-UPS")} title="Be ready for the next question">
              <div className="followup-list">
                {followups.map((p, i) => {
                  const answer = getFollowupAnswer(q.id, i, p);
                  return (
                    <details className="followup-item" key={`${q.id}-followup-${i}`}>
                      <summary>{p}</summary>
                      <div className="followup-answer">
                        <strong>Answer</strong>
                        <p>{answer || "No answer is mapped for this follow-up yet."}</p>
                      </div>
                    </details>
                  );
                })}
          ...
          <div className="question-review-bar">
            <div>
              <span>{isReviewed ? 'REVIEWED' : 'READY TO REVIEW'}</span>
              <small>{isReviewed ? 'You marked this question as reviewed.' : 'Mark it reviewed once you can explain the answer and handle the follow-up.'}</small>
            </div>
            <button type="button" className={isReviewed ? 'review-btn reviewed' : 'review-btn'} onClick={onReview}>{isReviewed ? '✓ Reviewed' : 'Mark reviewed'}</button>
          </div>
