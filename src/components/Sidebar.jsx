    ...
    <>
      ...
      <aside className={open ? "sidebar sidebar-open" : "sidebar"}>
        ...
        <SidebarScroll>
          ...
          <nav className="topic-nav">
            <button className={view === "docker" ? "nav-item active" : "nav-item"} onClick={() => go("docker")}>
              <span>🐳</span><label>Docker</label><small>{dockerLessons.length + dockerLabs.length}</small>
            </button>
            <button className={view === "kubernetes" ? "nav-item active" : "nav-item"} onClick={() => go("kubernetes")}>
              <span>☸</span><label>Kubernetes</label><small>{kubernetesLessons.length + kubernetesLabs.length}</small>
            </button>
            <button className={view === "cicd" ? "nav-item active" : "nav-item"} onClick={() => go("cicd")}>
              <span>⚙</span><label>CI/CD</label><small>{cicdLessons.length + cicdLabs.length}</small>
            </button>
            <button className={view === "jenkins" ? "nav-item active" : "nav-item"} onClick={() => go("jenkins")}>
              <span>🔧</span><label>Jenkins</label><small>{jenkinsLessons.length + jenkinsLabs.length}</small>
            </button>
            <button className={view === "github-actions" ? "nav-item active" : "nav-item"} onClick={() => go("github-actions")}>
          ...
          <nav className="topic-nav">
            <button className={view === "deep-dive" ? "nav-item active" : "nav-item"} onClick={() => go("deep-dive")}>
              <span>🧠</span><label>Deep Dive & Architecture</label><small>{deepDiveLessons.length + deepDiveLabs.length}</small>
            </button>
          </nav>

          <div className="side-label section-label advanced-label">INTERNAL WORKINGS</div>
          <nav className="topic-nav">
            <button className={view === "internal" ? "nav-item active" : "nav-item"} onClick={() => go("internal")}>
              <span>⚙</span><label>Internal Workings</label><small>{internalWorkingsLessons.length + internalWorkingsLabs.length}</small>
            </button>
          </nav>
          ...
          <nav className="topic-nav">
            ...
            <button className={view === "scenarios" ? "nav-item active" : "nav-item"} onClick={() => go("scenarios")}>
              <span>▱</span><label>Full Stack Scenarios</label><small>{scenarioFoundations.length + fullStackScenarios.length}</small>
            </button>
          </nav>
        </SidebarScroll>
