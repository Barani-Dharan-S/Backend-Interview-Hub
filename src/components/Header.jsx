...
export default function Header({ view }) {
  ...
  return (
    <header className="topbar">
      <div>
        <div className="eyebrow">{roadmap ? "fullstack@roadmap:~" : react ? "frontend@learning:~" : javascript ? "javascript@learning:~" : fullstack ? "fullstack@learning:~" : scenarios ? "production@scenarios:~" : labs ? "engineering@labs:~" : docker ? "devops@docker:~" : kubernetes ? "devops@kubernetes:~" : cicd ? "devops@cicd:~" : jenkins ? "devops@jenkins:~" : githubActions ? "devops@actions:~" : cloud ? "cloud@deployment:~" : production ? "production@engineering:~" : deepDive ? "architecture@deep-learning:~" : internal ? "runtime@system-behavior:~" : "backend@interview:~"}</div>
        <h1>{title}</h1>
        <p>{description}</p>
      </div>
