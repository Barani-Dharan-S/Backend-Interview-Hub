...
function LessonCard({ lesson, open, onToggle }) {
  return <article className="lesson-card">
    ...
    {open && <div className="lesson-body">
      ...
      <div className="explain-section"><div className="section-kicker">EXPECTED FOLLOW-UPS</div><div className="followup-list">{lesson.followups.map((x,i)=>{const answer=getFollowupAnswer(lesson.id,i,x);return <details className="followup-item" key={lesson.id+'-followup-'+i}><summary>{x}</summary><div className="followup-answer"><strong>Answer</strong><p>{answer || 'No answer is mapped for this follow-up yet.'}</p></div></details>})}</div></div>
