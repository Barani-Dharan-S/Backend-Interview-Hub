...
import DistributedSystemsLearning from "./pages/DistributedSystemsLearning";
...
export default function App() {
  ...
  return (
    <div className={dark ? "app dark" : "app light"}>
      ...
      <main className="main">
        <Header view={view} />
        {view === "roadmap" ? <Roadmap questions={questions} onTopic={openTopic} /> : view === "react" ? <ReactLearning /> : view === "javascript" ? <JavaScriptLearning /> : view === "fullstack" ? <FullStackLearning /> : view === "scenarios" ? <FullStackScenarios /> : view === "labs" ? <PracticalLabs /> : view === "docker" ? <DockerLearning /> : view === "kubernetes" ? <KubernetesLearning /> : view === "cicd" ? <CICDLearning /> : view === "jenkins" ? <JenkinsLearning /> : view === "github-actions" ? <GitHubActionsLearning /> : view === "cloud" ? <CloudLearning /> : view === "production" ? <ProductionEngineeringLearning /> : view === "deep-dive" ? <DeepDiveLearning /> : view === "internal" ? <InternalWorkingsLearning /> : view === "distributed" ? <DistributedSystemsLearning /> : <Questions questions={questions} topic={topic} subtopic={subtopic} setSubtopic={setSubtopic} />}
      </main>
    </div>
  );
}
